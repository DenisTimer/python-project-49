...
def get_progression(start, step, length):
    ...
    for index in range(length):
        ellement = start + index * step
        if index == index_answer:
            progression.append('..')
            correct_answer = ellement
        else:
            progression.append(str(ellement))
...
def question_and_answer_progression():
    ...
    qwestion_text, correct_answer = get_progression(start, step, length)
    return ' '.join(qwestion_text), str(correct_answer)